...
function autoFocus() {
    ...
    //轮播图
    var oneImg = document.querySelector('.topImg .one');
    var twoImg = document.querySelector('.topImg .two');
    var threeImg = document.querySelector('.topImg .three');
    var fourImg = document.querySelector('.topImg .four');
    var dot = document.querySelectorAll('.dotList a');
    var run = function () {
        setTimeout(function () {
            ...
            dot[0].style.backgroundColor = '#ccc';
            dot[1].style.backgroundColor = '#55a532';
            dot[2].style.backgroundColor = '#ccc';
            dot[3].style.backgroundColor = '#ccc';
            setTimeout(function () {
                oneImg.style.display = 'none';
                ...
                fourImg.style.display = 'none';
                dot[0].style.backgroundColor = '#ccc';
                dot[1].style.backgroundColor = '#ccc';
                dot[2].style.backgroundColor = '#55a532';
                dot[3].style.backgroundColor = '#ccc';
                setTimeout(function () {
                    oneImg.style.display = 'none';
                    twoImg.style.display = 'none';
                    ...
                    dot[1].style.backgroundColor = '#ccc';
                    dot[2].style.backgroundColor = '#ccc';
                    dot[3].style.backgroundColor = '#55a532';
                    setTimeout(function () {
                        oneImg.style.display = 'block';
                        twoImg.style.display = 'none';
                        threeImg.style.display = 'none';
                        fourImg.style.display = 'none';
                        dot[0].style.backgroundColor = '#55a532';
                        dot[1].style.backgroundColor = '#ccc';
                        dot[2].style.backgroundColor = '#ccc';
                        dot[3].style.backgroundColor = '#ccc';
                    }, 5000);
...
function reg_qq() {
    ...
    if (document.querySelector("#nickname").value !== '') {//防止先注册过qq，未刷新直接切换到邮箱注册，昵称会有重复，但是未手动触发onblur事件导致昵称的重复
        warnChange('.warn.nickname', 3);
    }
}
//邮箱注册
function reg_email() {

    document.querySelector("#email_name").focus();  //不起作用？？？？？？？？？？？？
...
function birthday_appear(obj_str) {
    ...
    if (obj_str == ".birthday_box2 ul") {
        for (var i = 2016; i > 1887; i--) {
            document.getElementById("year").innerHTML += '<li value="' + i + '" id="year_' + (2016 - i) + '" onmousedown="change_birth_type(' + "'year_value','" + i + "年'" + ')">' + i + '年</li>';
        }
    }
    if (obj_str == ".birthday_box3 ul") {
        ...
        if (yearValue.value.match(/\d+/g) == null || yearValue.value.match(/\d+/g) == "") {
            ...
            for (var j = 1; j < 13; j++) {
                theMonth.innerHTML += '<li value="' + j + '" id="month_' + (j - 1) + '" onmousedown="change_birth_type(' + "'month_value','" + j + "月'" + ')">' + j + '月</li>';
            }
        }
    }
    ...
    if (obj_str == ".birthday_box4 ul") {
        ...
        if (monthValue.value.match(/\d+/g) == 1 || monthValue.value.match(/\d+/g) == 3 || monthValue.value.match(/\d+/g) == 5 || monthValue.value.match(/\d+/g) == 7 || monthValue.value.match(/\d+/g) == 8 || monthValue.value.match(/\d+/g) == 10 || monthValue.value.match(/\d+/g) == 12) {
            ...
            for (var k = 1; k < 32; k++) {
                theDay.innerHTML += '<li value="' + k + '" id="year_' + (k - 1) + '" onmousedown="change_birth_type(' + "'day_value','" + k + "日'" + ')">' + k + '日</li>';
            }
